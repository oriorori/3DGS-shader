...
half4 frag (v2f i) : SV_Target
{
	...
	// 이쯤에 아마 조명 계산 추가해야할거에요..
	// phong reflection model이니까
    // float3 lightDir = normalize(_LightDirection.xyz);
    // float3 normal = float3(0, 0, 1); // 기본 노말 값 (스플랫은 화면 상에 평평한 원형으로 가정) //아마 이거 벡터가 안이뻐서 조명이 이상하게 나올수도!
	// float3 viewDir = normalize(_WorldSpaceCameraPos - i.worldPos); // 카메라에서 스플랫까지의 방향
